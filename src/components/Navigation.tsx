    ...
    <header className="bg-card border-b border-border/20 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-6 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2 sm:space-x-4">
            ...
            {variant === 'dashboard' && (
              <button
                ...
                className="sm:hidden p-2 rounded-lg hover:bg-muted/10 transition-colors"
              ...
              {isProfileDropdownOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-card rounded-xl shadow-lg border border-border/60 overflow-hidden">
                  ...
                  <div className="p-2">
                    ...
                    {/* Admin link - only show for specific users */}
                    {(user?.email === 'admin@dealsletter.com' || user?.email === 'kevin@dealsletter.com') && (
                      <Link 
          ...
          {/* Mobile Menu Button - only for non-dashboard variant */}
          {variant !== 'dashboard' && (
          ...
          {/* Mobile User Avatar - for dashboard variant */}
          {variant === 'dashboard' && user && (
            <div className="md:hidden">
              <button
                onClick={() => setIsProfileDropdownOpen(!isProfileDropdownOpen)}
                className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold text-sm ${getTierColor()}`}
              >
                {getInitials()}
              </button>
            </div>
            ...
            <div className="flex flex-col space-y-3">
              {variant === 'dashboard' ? (
                <>
                  ...
                  <Link 
                    href="/dashboard" 
                    className="px-6 py-3 text-primary bg-primary/10 rounded-lg font-medium min-h-[44px] flex items-center gap-3"
