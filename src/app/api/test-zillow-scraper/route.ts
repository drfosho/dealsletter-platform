...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!url.includes('zillow.com')) {
      return NextResponse.json(
        { error: 'Invalid URL: Must be a Zillow property URL' },
        { status: 400 }
      );
    }

    console.log(`[API] Testing Zillow scraper for: ${url}`);
    ...
    if (!result.success) {
      return NextResponse.json(
        { 
          success: false,
          error: result.error,
          message: 'Failed to scrape Zillow property'
        },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'Successfully scraped Zillow property',
      data: result.data,
      debug: {
        url,
    ...
    console.error('[API] Zillow scraper error:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const results = await Promise.allSettled(
      urls.map(url => apifyService.scrapeZillowProperty(url))
    );
    ...
    console.error('[API] Bulk Zillow scraper error:', error);
    return NextResponse.json(
      { 
        success: false,
