-- Property Analysis Feature Database Setup (Fixed Version)
...
DO $$ 
BEGIN
    -- Drop policies for analyzed_properties if table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'analyzed_properties') THEN
        DROP POLICY IF EXISTS "Users can view their own analyzed properties" ON analyzed_properties;
        DROP POLICY IF EXISTS "Users can create their own analyzed properties" ON analyzed_properties;
        DROP POLICY IF EXISTS "Users can update their own analyzed properties" ON analyzed_properties;
        DROP POLICY IF EXISTS "Users can delete their own analyzed properties" ON analyzed_properties;
    END IF;
    
    -- Drop policies for usage_tracking if table exists
...
DROP TABLE IF EXISTS analyzed_properties CASCADE;
...
DROP FUNCTION IF EXISTS get_or_create_usage(UUID, TEXT) CASCADE;
...
DO $$
BEGIN
  RAISE NOTICE 'Property analysis database setup completed successfully!';
END $$;